...
**Status: in_progress**
**Current checkpoint:** CP-182 — Transport-failure cleanup locally green
**Last checkpoint (UTC):** 2026-08-31T13:43:20Z
**Active phase:** Phase 1 — Rust media engine
**Active milestone:** Transport failure cleanup and Asterisk fallback<br>
**Next resume action:** Commit and publish the transport-failure cleanup slice, then verify hosted PR #61 checks while retaining the provider/Asterisk evidence gate
**Active PR:** #61 — `transport-failure-cleanup` targets `graceful-drain` (local checks green; publication pending)
...
| Transport failure cleanup | in_progress | CP-182; `fail_active_calls` and runtime transport-loss handling transition non-terminal calls to terminal state, release signaling/retry resources, synchronize bridge/provider context, and retain bounded terminal records; focused call-core/engine/runtime tests, 282 locked workspace tests, format, Clippy, three pinned Docker-backed SIPp scenarios, deterministic signaling/media/WebSocket/combined/short-soak smokes, and six-target nightly ASAN fuzz checks pass locally | [#61](https://github.com/W3Mirror/asterisk/pull/61) pending publication | Commit/push and verify the complete ordinary hosted suite |
