...
**Status: in_progress**
**Current checkpoint:** CP-183 — PR #61 hosted validation green
**Last checkpoint (UTC):** 2026-08-31T13:49:30Z
...
| Transport failure cleanup | hosted_green | CP-182–CP-183; `fail_active_calls` and runtime transport-loss handling transition non-terminal calls to terminal state, release signaling/retry resources, synchronize bridge/provider context, and retain bounded terminal records; focused call-core/engine/runtime tests, 282 locked workspace tests, format, Clippy, three pinned Docker-backed SIPp scenarios, deterministic signaling/media/WebSocket/combined/short-soak smokes, and six-target nightly ASAN fuzz checks pass locally; hosted PR #61 ordinary suite passed | [#61](https://github.com/W3Mirror/asterisk/pull/61) OPEN/CLEAN/MERGEABLE at `0a15536d8` | Continue the next bounded offline acceptance slice |
