...
**Status: in_progress**
**Current checkpoint:** CP-211 — PR #69 hosted validation green
**Last checkpoint (UTC):** 2026-08-31T18:33:02Z
...
| Configuration-level route reload | hosted_green | CP-210–CP-211; generation-checked atomic `ProviderRouteTable` replacement, fail-closed Asterisk reset after changed configuration, idempotent identical reloads, explicit Rust reactivation, stale-generation and exhaustion safety, and runtime no-resolution/no-mutation coverage; focused provider-routing (11 tests) and call-runtime (48 tests), complete locked workspace tests, formatting, workspace Clippy, three pinned Docker-backed SIPp scenarios, deterministic signaling/media/WebSocket/combined/short-soak reclamation smokes, all fuzz-target checks, and dependency audit pass; hosted run `33425289168` is green; `docs/rust-route-config-reload.md` | [#69](https://github.com/W3Mirror/asterisk/pull/69) OPEN/CLEAN/MERGEABLE at `38f6140e7` | Continue the next bounded offline acceptance slice without enabling Rust traffic |
