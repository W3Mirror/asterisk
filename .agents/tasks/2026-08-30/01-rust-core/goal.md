...
| 43 | pending [#43](https://github.com/W3Mirror/asterisk/pull/43) | `signaling-capacity-matrix` | `websocket-load-smoke` | `/home/ashutosh/.worktrees/w3mirror/asterisk/pr-43` | Exact scheduled 1,000/5,000/10,000 in-memory signaling concurrency matrix with bounded logical reclamation and best-effort process observations | locally green | `782cacd39` before the implementation/checkpoint commit | CP-119; two directly relevant regressions, all 12 load-smoke tests, and all 215 workspace tests pass; the exact matrix, all existing ordinary/scheduled smokes, SIPp, formatting, strict/package and workspace Clippy, workflow/script parsing, six sanitizer fuzz-target checks, and diff checks pass | Commit and publish stacked PR #43 against exact base `websocket-load-smoke`, then verify the ordinary hosted suite on its final PR head |
