...
impl CallRuntime {
    ...
    /// Starts an outbound human leg after verifying transfer permission.
    ///
    /// Transfer permission authorizes the underlying second-leg origination;
    /// callers do not need a separate originate grant for this implementation
    /// detail. Authorization is checked before bridge lookup, call allocation,
    /// address use, or transport delivery. The engine and bridge registry are
    /// unchanged when authorization, bridge validation, or delivery fails.
    pub fn originate_human_leg_authorized(
        &mut self,
        principal: &AuthenticatedPrincipal,
        bridge_id: &BridgeId,
        leg_id: LegId,
        ...
        if let Err(error) = principal.authorize(CallCommand::BeginTransfer) {
            self.engine.record_bridge_audit(
                principal,
                None,
                BridgeControlOperation::OriginateHuman,
                AuditOutcome::Rejected(error.code()),
            );
            return Err(RuntimeError::Engine(EngineError::CallApi(error)));
        }
        let result = self.originate_human_leg(bridge_id, leg_id, request, destination, now);
        let call_id = result
            .as_ref()
            .ok()
            .map(|(call_id, _)| call_id.clone())
            .or_else(|| bridge_call_id(self.bridges.as_ref(), bridge_id));
        self.engine.record_bridge_audit(
            principal,
            call_id.as_ref(),
            BridgeControlOperation::OriginateHuman,
            result.as_ref().map_or_else(
                |error| AuditOutcome::Rejected(runtime_error_code(error)),
    ...
    /// Completes a pending human bridge transition transactionally.
    ///
    /// Normal SIP answer processing completes this transition automatically.
    /// This method is provided for a trusted supervisor that has already
    /// established the human leg through another signaling boundary.
    pub fn complete_human(&mut self, bridge_id: &BridgeId) -> Result<RuntimeOutput, RuntimeError> {
        self.apply_bridge_transition(|bridges| bridges.complete_human(bridge_id))
    }

    /// Completes a pending human bridge transition after transfer authorization.
    pub fn complete_human_authorized(
        &mut self,
        principal: &AuthenticatedPrincipal,
        bridge_id: &BridgeId,
    ) -> Result<RuntimeOutput, RuntimeError> {
        self.apply_authorized_bridge_transition(
            principal,
            bridge_id,
            CallCommand::CompleteTransfer,
            BridgeControlOperation::CompleteHuman,
            move |bridges| bridges.complete_human(bridge_id),
        )
    }

    /// Fails a pending or active human leg and restores AI routing
    /// transactionally.
    pub fn fail_human(&mut self, bridge_id: &BridgeId) -> Result<RuntimeOutput, RuntimeError> {
        self.apply_bridge_transition(|bridges| bridges.fail_human(bridge_id))
    }

    /// Fails a human leg after transfer authorization.
    pub fn fail_human_authorized(
        &mut self,
        principal: &AuthenticatedPrincipal,
        bridge_id: &BridgeId,
    ) -> Result<RuntimeOutput, RuntimeError> {
        self.apply_authorized_bridge_transition(
            ...
            BridgeControlOperation::FailHuman,
            move |bridges| bridges.fail_human(bridge_id),
        )
    }

    /// Restores AI routing from an active human bridge transactionally.
    pub fn resume_ai(&mut self, bridge_id: &BridgeId) -> Result<RuntimeOutput, RuntimeError> {
        self.apply_bridge_transition(|bridges| bridges.resume_ai(bridge_id))
    }

    /// Restores AI routing after transfer authorization.
    pub fn resume_ai_authorized(
        &mut self,
        principal: &AuthenticatedPrincipal,
        bridge_id: &BridgeId,
    ) -> Result<RuntimeOutput, RuntimeError> {
        self.apply_authorized_bridge_transition(
            ...
            bridge_id,
            CallCommand::CompleteTransfer,
            BridgeControlOperation::ResumeAi,
            move |bridges| bridges.resume_ai(bridge_id),
        )
    }

    /// Ends bridge forwarding and releases its retained human endpoints.
    pub fn end_bridge(&mut self, bridge_id: &BridgeId) -> Result<RuntimeOutput, RuntimeError> {
        self.end_bridge_at(bridge_id, Duration::ZERO)
    }

    /// Ends bridge forwarding at a deterministic time, sending BYE to any
    /// active or pending human leg before releasing the bridge endpoints.
    pub fn end_bridge_at(
        &mut self,
        bridge_id: &BridgeId,
        now: Duration,
    ) -> Result<RuntimeOutput, RuntimeError> {
        let mut working_bridges = self
            ...
            .clone()
            .ok_or(RuntimeError::BridgeRegistryNotConfigured)?;
        let snapshot = working_bridges.snapshot(bridge_id)?;
        let human_calls = [snapshot.pending_human, snapshot.active_human]
            .into_iter()
            .flatten()
            .map(|human| human.call_id)
            .collect::<Vec<_>>();
        let mut working_engine = self.engine.clone();
        let mut output = RuntimeOutput::default();
        for call_id in human_calls {
            let state = working_engine.snapshot(&call_id)?.state;
            if matches!(
                state,
                call_core::CallState::Ended | call_core::CallState::Failed
            ) {
                continue;
            }
            output.append(working_engine.hangup(&call_id, now, self.transport.reliability())?);
        }
        let _ = working_bridges.end(bridge_id)?;
        output
            .bridge_events
            .extend(working_bridges.drain_events(usize::MAX)?);
    ...
    /// Ends bridge forwarding after hangup authorization.
    pub fn end_bridge_authorized(
        &mut self,
        principal: &AuthenticatedPrincipal,
        bridge_id: &BridgeId,
    ) -> Result<RuntimeOutput, RuntimeError> {
        if let Err(error) = principal.authorize(CallCommand::Hangup) {
            self.engine.record_bridge_audit(
                principal,
                None,
                BridgeControlOperation::EndBridge,
                AuditOutcome::Rejected(error.code()),
            );
            return Err(RuntimeError::Engine(EngineError::CallApi(error)));
        }
        let call_id = bridge_call_id(self.bridges.as_ref(), bridge_id);
        let result = self.end_bridge(bridge_id);
        self.engine.record_bridge_audit(
            principal,
            call_id.as_ref(),
            BridgeControlOperation::EndBridge,
    ...
    /// Sends one application-controlled request inside a confirmed dialog
    /// through the configured SIP transport.
    ///
    /// Dialog identity, routing, `CSeq`, and transaction branch are generated by
    /// [`CallEngine`]. The runtime only supplies the transport reliability and
    /// atomically delivers the resulting action.
    ///
    /// # Errors
    ///
    /// Returns an error when the call engine rejects the request or the
    /// configured transport cannot deliver it. Neither the runtime engine nor
    /// its wire-visible state is committed when delivery fails.
    pub fn send_in_dialog_request(
        ...
        let mut working_engine = self.engine.clone();
        let output = working_engine.send_in_dialog_request(
            call_id,
            method,
            extra_headers,
            body,
    ...
    /// Sends an in-dialog request after verifying control-plane permission
    /// and records the bounded authorization outcome.
    ///
    /// # Errors
    ///
    /// Returns an error when authorization, request validation, or transport
    /// delivery fails. Authorization audit records are retained even when the
    /// request is rejected.
    pub fn send_in_dialog_request_authorized(
        ...
        let mut working_engine = self.engine.clone();
        let result = working_engine.send_in_dialog_request_authorized(
            principal,
            call_id,
            method,
            extra_headers,
        ...
        match result {
            ...
            Err(error) => {
                // Keep the audit-only mutation from a rejected authorized
                // operation, while no wire action or lifecycle event is
    ...
    pub fn prepare_restart_handoff(&mut self) -> Result<RuntimeRestartHandoff, RuntimeError> {
        ...
        if let Some(bridges) = working_bridges.as_mut() {
            ...
            for event in engine_handoff.events() {
                synchronize_bridge_events(bridges, std::slice::from_ref(event))?;
                bridge_events.extend(bridges.drain_events(usize::MAX)?);
            }
    ...
    fn commit_runtime_output(
        &mut self,
        mut working_engine: CallEngine,
        mut output: RuntimeOutput,
    ) -> Result<RuntimeOutput, RuntimeError> {
        let mut working_bridges = self.bridges.clone();
        let mut working_provider_authentication = self.provider_authentication.clone();
        if let Some(bridges) = working_bridges.as_mut() {
            let lifecycle_events = output.events.clone();
            synchronize_bridge_lifecycle(
                &mut working_engine,
                bridges,
                &lifecycle_events,
                self.transport.reliability(),
    ...
    fn apply_bridge_transition<F>(&mut self, transition: F) -> Result<RuntimeOutput, RuntimeError>
    where
        F: FnOnce(&mut BridgeRegistry) -> Result<BridgeEvent, BridgeError>,
    {
        let mut working_bridges = self
            .bridges
            .clone()
            .ok_or(RuntimeError::BridgeRegistryNotConfigured)?;
        transition(&mut working_bridges)?;
        let mut output = RuntimeOutput::default();
        output
            .bridge_events
            .extend(working_bridges.drain_events(usize::MAX)?);
    ...
    fn apply_authorized_bridge_transition<F>(
        &mut self,
        principal: &AuthenticatedPrincipal,
        bridge_id: &BridgeId,
        permission_command: CallCommand,
        ...
        F: FnOnce(&mut BridgeRegistry) -> Result<BridgeEvent, BridgeError>,
    {
        ...
        let call_id = bridge_call_id(self.bridges.as_ref(), bridge_id);
        let result = self.apply_bridge_transition(transition);
        self.engine.record_bridge_audit(
            principal,
            call_id.as_ref(),
            operation,
...
fn runtime_error_code(error: &RuntimeError) -> &'static str {
    match error {
        RuntimeError::Engine(EngineError::CallApi(error)) => error.code(),
        RuntimeError::Bridge(_) => "bridge_error",
        RuntimeError::Engine(_) => "engine_error",
        RuntimeError::Transport(_) => "transport_error",
        RuntimeError::DestinationMismatch { .. } => "destination_mismatch",
        RuntimeError::ConnectionClosed { .. } => "connection_closed",
        RuntimeError::SourceAddressDenied { .. } => "source_address_denied",
        RuntimeError::NotDatagram => "not_datagram",
        RuntimeError::BridgeRegistryNotConfigured => "bridge_registry_not_configured",
        RuntimeError::ProviderRouteFallback { .. } => "provider_route_fallback",
        RuntimeError::ProviderTransportUnsupported => "provider_transport_unsupported",
        RuntimeError::ProviderAddressUnavailable => "provider_address_unavailable",
    }
}

fn bridge_call_id(bridges: Option<&BridgeRegistry>, bridge_id: &BridgeId) -> Option<CallId> {
    let snapshot = bridges?.snapshot(bridge_id).ok()?;
    snapshot
        .active_human
        .as_ref()
        .or(snapshot.pending_human.as_ref())
        .map(|human| human.call_id.clone())
        .or(Some(snapshot.caller_call_id))
}

fn synchronize_bridge_events(
...
fn synchronize_bridge_lifecycle(
    ...
    for event in events {
        let caller_bridge = bridges
            .list(bridges.config().max_bridges)?
            .into_iter()
            .find(|bridge| bridge.caller_call_id == event.call_id);
        if let Some(snapshot) = caller_bridge {
            if matches!(
                event.kind,
                CallEventKind::Hangup | CallEventKind::Failed | CallEventKind::Ended
            ) && snapshot.state != BridgeState::Ended
            {
                let human_calls = [snapshot.pending_human, snapshot.active_human]
                    .into_iter()
                    .flatten()
                    ...
                    .collect::<Vec<_>>();
                for call_id in human_calls {
                    let state = engine.snapshot(&call_id)?.state;
                    if !matches!(
                        state,
                        call_core::CallState::Ended | call_core::CallState::Failed
                    ) {
                        output.append(engine.hangup(&call_id, Duration::ZERO, reliability)?);
                    }
                }
...
mod tests {
    use super::*;
    use call_api::{
        ApiError, AuditOperation, AuditOutcome, BridgeControlOperation, ControlPermission,
    };
    ...
    fn response_for_in_dialog_request(request: &SipRequest) -> SipMessage {
        let mut headers = Headers::new();
        headers.push("Via", request.headers.get("Via").unwrap());
        headers.push("From", request.headers.get("From").unwrap());
    ...
    #[test]
    fn udp_runtime_delivers_outbound_in_dialog_request_and_processes_response() {
        let runtime_transport = UdpTransport::bind("127.0.0.1:0".parse().unwrap(), 2_048).unwrap();
        let runtime_address = runtime_transport.local_addr().unwrap();
        let peer = UdpTransport::bind("127.0.0.1:0".parse().unwrap(), 2_048).unwrap();
        ...
        let (call_id, _) = runtime
            .originate(invite(), peer_address, Duration::ZERO)
            .unwrap();
        let (initial_message, _) = peer.recv().unwrap();
        let SipMessage::Request(initial_request) = initial_message else {
            panic!("expected outbound INVITE");
        };
        peer.send_to(&response_for_invite(&initial_request, 200), runtime_address)
            .unwrap();
        let answered = runtime.receive_once(Duration::from_millis(1)).unwrap();
        assert!(
            answered
                .events()
                .iter()
        ...
        extra_headers.push("Info-Package", "dtmf");
        let sent = runtime
        ...
        let (request_message, source) = peer.recv().unwrap();
        assert_eq!(source, runtime_address);
        let SipMessage::Request(request) = request_message else {
            panic!("expected outbound INFO");
        };
        assert_eq!(request.method, SipMethod::Info);
        assert_eq!(
            request.headers.get("Call-ID"),
            Some("runtime-invite@example.com")
        );
        assert_eq!(request.headers.get("CSeq"), Some("2 INFO"));
        assert_eq!(request.headers.get("Info-Package"), Some("dtmf"));
        assert_eq!(request.body, b"Signal=5\r\nDuration=160");

        peer.send_to(&response_for_in_dialog_request(&request), runtime_address)
            .unwrap();
        let completed = runtime.receive_once(Duration::from_millis(3)).unwrap();
        assert!(completed.actions().is_empty());
        assert_eq!(
            runtime.engine().snapshot(&call_id).unwrap().state,
            CallState::Answered
        );
        assert_eq!(runtime.engine().transaction_count(), 1);

        let expired = runtime.poll(Duration::from_secs(8)).unwrap();
        assert!(expired.actions().is_empty());
        assert_eq!(runtime.engine().transaction_count(), 0);
        assert_eq!(runtime.engine().dialog_count(), 1);
    }
    ...
    #[test]
    fn ending_active_bridge_sends_bye_and_waits_for_human_terminal_response() {
        ...
        let ending = runtime
            .end_bridge_at(&bridge_id, Duration::from_millis(20))
            .unwrap();
        assert_eq!(ending.bridge_events()[0].kind, BridgeEventKind::Ended);
        assert!(ending.actions().iter().any(|action| matches!(
            action.message,
            SipMessage::Request(SipRequest {
                method: SipMethod::Bye,
                ..
        ...
        let (message, _) = human.recv().unwrap();
        let SipMessage::Request(bye) = message else {
            panic!("expected human-leg BYE");
        };
        ...
        human
            .send_to(&response_for_invite(&bye, 200), runtime_address)
            .unwrap();
        let completed = runtime.receive_once(Duration::from_millis(30)).unwrap();
        assert!(
            completed.events().iter().any(|event| {
                event.call_id == human_call_id && event.kind == CallEventKind::Ended
            })
        );
        assert_eq!(
            runtime.engine().snapshot(&human_call_id).unwrap().state,
    ...
    #[test]
    fn caller_hangup_ends_bridge_and_sends_bye_to_active_human_leg() {
        let (mut runtime, bridge_id, caller_id) = runtime_with_inbound_bridge();
        let inbound = inbound_invite();
        let answered = runtime
            .respond_to_invite(&caller_id, 200, "OK", Vec::new(), Duration::ZERO)
            .unwrap();
        let SipMessage::Response(response) = &answered.actions()[0].message else {
            panic!("expected inbound 200 response");
        };
        let caller = UdpTransport::bind("127.0.0.1:0".parse().unwrap(), 2_048).unwrap();
        let mut ack_headers = Headers::new();
        ack_headers.push(
            "Via",
            "SIP/2.0/UDP caller.invalid;branch=caller-confirm-ack",
        );
        ack_headers.push("From", inbound.headers.get("From").unwrap());
        ack_headers.push("To", response.headers.get("To").unwrap());
        ack_headers.push("Call-ID", inbound.headers.get("Call-ID").unwrap());
        ack_headers.push("CSeq", "1 ACK");
        caller
            .send_to(
                &SipMessage::Request(SipRequest {
                    method: SipMethod::Ack,
                    request_uri: inbound.request_uri.clone(),
                    version: "SIP/2.0".to_owned(),
                    headers: ack_headers,
                    body: Vec::new(),
                }),
                runtime.transport().local_addr().unwrap(),
            )
            .unwrap();
        let _ = runtime.receive_once(Duration::from_millis(1)).unwrap();
        let human = UdpTransport::bind("127.0.0.1:0".parse().unwrap(), 2_048).unwrap();
        let runtime_address = runtime.transport().local_addr().unwrap();
        let (human_call_id, _) = runtime
            .originate_human_leg(
        ...
        headers.push(
            "Via",
            "SIP/2.0/UDP caller.invalid;branch=caller-terminal-bye",
        );
        headers.push("From", "Alice <sip:alice@example.com>;tag=client-2");
        headers.push("To", "Bob <sip:bob@example.com>;tag=rust-1");
        ...
        caller
            ...
            .unwrap();
        let ended = runtime.receive_once(Duration::from_millis(20)).unwrap();
        assert_eq!(
            runtime
                .bridge_registry()
                .unwrap()
        ...
        assert_eq!(
            runtime.engine().snapshot(&caller_id).unwrap().state,
            CallState::Ended
        );
        assert_eq!(
            runtime.engine().snapshot(&human_call_id).unwrap().state,
        ...
        assert!(ended.actions().iter().any(|action| matches!(
            action.message,
            SipMessage::Request(SipRequest {
                method: SipMethod::Bye,
                ..
    ...
    #[test]
    fn authorized_human_leg_checks_transfer_permission_before_bridge_lookup() {
        let (mut runtime, bridge_id, _) = runtime_with_inbound_bridge();
        let read_only = principal(&[ControlPermission::ReadCalls]);
        let calls_before = runtime.engine().list(16).unwrap();
        let bridge_before = runtime
            .bridge_registry()
            .unwrap()
            .snapshot(&bridge_id)
            .unwrap();
        ...
        let error = runtime
            .originate_human_leg_authorized(
                &read_only,
                &bridge_id,
                LegId::from_sequence(2),
                invite(),
        ...
        assert!(matches!(
            error,
            RuntimeError::Engine(EngineError::CallApi(ApiError::PermissionDenied {
                command: CallCommand::BeginTransfer,
                permission: ControlPermission::TransferCalls,
            }))
        ));
        assert_eq!(runtime.engine().list(16).unwrap(), calls_before);
        assert_eq!(
            runtime
                .bridge_registry()
                .unwrap()
        ...
        let mut buffer = [0_u8; 1];
        let receive_error = peer.recv(&mut buffer).unwrap_err();
        assert!(matches!(
            receive_error.kind(),
            std::io::ErrorKind::WouldBlock | std::io::ErrorKind::TimedOut
        ));
    ...
    #[test]
    fn authorized_bridge_controls_restore_ai_and_end_forwarding() {
        let (mut runtime, bridge_id, caller_id) = runtime_with_inbound_bridge();
        ...
        let (human_call_id, connecting) = runtime
            .originate_human_leg_authorized(
                &transfer,
                &bridge_id,
                LegId::from_sequence(2),
        ...
        human
            .send_to(&response_for_invite(&outbound_invite, 200), runtime_address)
            .unwrap();
        let answered = runtime.receive_once(Duration::from_millis(10)).unwrap();
        assert_eq!(
            answered.bridge_events()[0].kind,
            BridgeEventKind::HumanConnected
        );
        ...
        assert_eq!(
            runtime
                ...
                .as_ref()
                .map(|human| human.call_id.clone()),
            Some(human_call_id.clone())
        );

        let denied = runtime
            .end_bridge_authorized(&read_only, &bridge_id)
            .unwrap_err();
        assert!(matches!(
            denied,
            RuntimeError::Engine(EngineError::CallApi(ApiError::PermissionDenied {
        ...
        assert_eq!(
            runtime
                ...
                .unwrap()
                .state,
            BridgeState::HumanActive
        );

        let resumed = runtime.resume_ai_authorized(&transfer, &bridge_id).unwrap();
        assert_eq!(resumed.bridge_events()[0].kind, BridgeEventKind::AiResumed);
        assert_eq!(
            runtime
                .bridge_registry()
                .unwrap()
        ...
        let ended = runtime.end_bridge_authorized(&hangup, &bridge_id).unwrap();
        assert_eq!(ended.bridge_events()[0].kind, BridgeEventKind::Ended);
        assert_eq!(
            runtime
                .bridge_registry()
                .unwrap()
        ...
        let audit = runtime.drain_audit_records(&read_only, 16).unwrap();
        assert!(audit.iter().any(|record| {
            record.operation
                == AuditOperation::BridgeControl {
                    operation: BridgeControlOperation::OriginateHuman,
                }
                && record.call_id == Some(human_call_id.clone())
                && record.outcome == AuditOutcome::Succeeded
        }));
        assert!(audit.iter().any(|record| {
            record.operation
                == AuditOperation::BridgeControl {
                    operation: BridgeControlOperation::EndBridge,
                }
                && record.outcome == AuditOutcome::Rejected("permission_denied")
        }));
        assert!(audit.iter().any(|record| {
            record.operation
                == AuditOperation::BridgeControl {
                ...
                && record.call_id == Some(caller_id.clone())
                && record.outcome == AuditOutcome::Succeeded
        }));
    }
}
